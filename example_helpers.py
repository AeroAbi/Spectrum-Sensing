...
def adqapi_load(path="", quiet=False):
    ...
    ADQAPI.ADQ_GetCardOption.restype = ct.c_char_p
...
def alternate_background(ax, start_point, widths, labels=False, color="#dddddd"):
    ...
    if labels:
        for idx in range(0, len(edges) - 1):
            ...
            # Enumerate fields
            plt.text(edges[idx], ypos, "R{}".format(idx), verticalalignment="top")
